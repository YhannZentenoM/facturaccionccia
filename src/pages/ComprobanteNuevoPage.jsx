...
const ComprobanteNuevoPage = () => {
    ...
    const operations = [
        ...
        { value: 4, label: 'ANTICIPO O DEDUCCIÓN DE ANTICIPO EN VENTA INTERNA [04]' },
        { value: 2, label: 'EXPORTACIÓN [02]' },
        { value: 29, label: 'VENTAS NO DOMICILIADOS QUE NO CALIFICAN COMO EXPORTACION [0401]' },
        { value: 30, label: 'OPERACIÓN SUJETA A DETRACCIÓN [1001]' },
        { value: 33, label: 'OPERACIÓN SUJETA A DETRACCIÓN - SERVICIOS DE TRANSPORTE CARGA [1004]' },
        { value: 34, label: 'OPERACIÓN SUJETA A PERCEPCIÓN [2001]' },
        { value: 32, label: 'OPERACIÓN SUJETA A DETRACCIÓN - SERVICIOS DE TRANSPORTE PASAJEROS [1003]' },
        { value: 31, label: 'OPERACIÓN SUJETA A DETRACCIÓN - RECURSOS HIDROBIOLÓGICOS [1002]' }
    ]
    const seriesData = [
    ...
    const filterSeriesData = () => {
        const series = seriesData.filter((item) => item.document_type_id == formData.tipo_de_comprobante)
        setSeries(series)
    ...
    const handleInputNumberChangeProductos = async (id, e, name) => {
        // const dataProducto = await getProducto(formData.items[+id - 1].producto_id)
        // const igv = decimalAdjust((+producto.precio_venta * 0.18), 10)
        // const valorUnitario = decimalAdjust(+producto.precio_venta - igv, 10)
        // const subtotal = decimalAdjust(+valorUnitario, 10)
        // const total = decimalAdjust(+subtotal + +igv, 10)
        const nuevasFilas = await Promise.all(filas.map(async (fila) => {
            // console.log("FILS", fila)
            // return fila.id === id ? {
            //     ...fila,
            //     [name]: +e.target.value,
            ...
            if (fila.id === id) {
                ...
                const igv = decimalAdjust((+dataProducto.precio_venta * 0.18), 2)
        ...
        <form>
            <div className='grid grid-cols-5 mt-2 gap-x-4 gap-y-2'>
                ...
                <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                    ...
                    <SelectInput
                        name={'serie'}
                        options={series}
                        onChange={(e) => { setFormData({ ...formData, serie: e.value }); setSelectedSerie(e) }}
            ...
            <div className='grid grid-cols-6 mt-2 gap-x-4 gap-y-2'>
                <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                    ...
                    <SelectInput
                        name={'tipoOperacion'}
                        options={operations}
                        onChange={(e) => { setFormData({ ...formData, sunat_transaction: e.value }); setSelectedOperaciones(e) }}
                ...
                <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                    ...
                    <input
                        ...
                        onChange={(e) => setFormData({ ...formData, fecha_de_emision: e.target.value })}
                ...
                <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                    ...
                    <SelectInput
                        name={'igv'}
                        options={igv}
                        onChange={(e) => { setFormData({ ...formData, porcentaje_de_igv: e.value }) }}
                        selected={igv[0]}
                    />
                </label>
                <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                ...
                <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                    Tipo de cambio
                    <input type="text" name='cambio' className='py-2 px-3 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-full text-zinc-900 rounded-lg' />
                </label>
            </div>
                        ...
                        <div key={fila.id} className='grid grid-cols-10 gap-1'>
                            ...
                            <label className='flex flex-col gap-1 text-sm text-zinc-500 col-span-2'>
                                <SelectInput
                                    ...
                                    options={productos}
                                    // onChange={(e) => { setFormData({ ...formData, moneda: e.value }); setSelectedMoneda(e) }}
                                    onChange={(e) => handleSelectChangeProductosData(fila.id, e, `producto_id`)}
                                // selected={selectedMoneda}
                                />
                            </label>
                            <label className='flex flex-col gap-1 text-sm text-zinc-500 col-span-2'>
                                <input
                                    ...
                                    className='py-2 px-3 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-full text-zinc-900 rounded-lg'
                                    onChange={(e) => handleInputChangeProductos(fila.id, e, `detalle`)}
                            ...
                            <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                                <input
                                    ...
                                    className='py-2 px-3 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-full text-zinc-900 rounded-lg'
                                    onChange={(e) => handleInputNumberChangeProductos(fila.id, e, `cantidad`)}
                                    defaultValue={1}
                                />
                            </label>
                            <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                                <SelectInput
                                    ...
                                    options={tipoIgv}
                                    onChange={(e) => handleSelectChangeProductos(fila.id, e, `tipo_de_igv`)}
                                    selected={tipoIgv[0]}
                                />
                            </label>
                            <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                                <input
                                    ...
                                    name='valor_unitario'
                                    className='py-2 px-3 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-full text-zinc-900 rounded-lg'
                                    onChange={(e) => handleInputChangeProductos(fila.id, e, `valor_unitario`)}
            ...
            <div className='grid grid-cols-3 mt-2 gap-x-4 gap-y-2'>
                <div className='col-span-2'>
                    <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                        Observaciones
                        <textarea name="observaciones" rows="3" className='py-1 px-2 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-full text-zinc-900 rounded-lg'></textarea>
                    </label>
                    ...
                    {
                        isActiveDetraccion && formData.tipo_operacion == 30 &&
                        <div className='grid grid-cols-3 gap-y-2 gap-x-4'>
                            ...
                            <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                                ...
                                <input type="text" name='porcentajeDetraccion' className='py-2 px-3 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-full text-zinc-900 rounded-lg' placeholder='%' />
                            </label>
                            <label className='flex flex-col col-span-2 gap-1 text-sm text-zinc-500'>
                                Medio de Pago
                                <SelectInput
                                    name={'moneda'}
                                    options={moneda}
                                    onChange={(e) => setFormData({ ...formData, tipo_operacion: e.value })}
                                    selected={formData.tipo_operacion[0]}
                                />
                            </label>
                            <label className='flex flex-col gap-1 text-sm text-zinc-500'>
                                Total de detracción S/
                                <input type="text" name='totalDetraccion' className='py-2 px-3 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-full text-zinc-900 rounded-lg' placeholder='S/' />
                            </label>
                        </div>
                    }
                </div>
                <div className='flex flex-col'>
                    <label className='flex items-center justify-end gap-3 text-sm text-zinc-500'>
                        % Descuento Global
                        <input type="text" className='py-1 px-2 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-[100px] lg:w-[150px] text-zinc-900 rounded-lg' />
                    </label>
                    <label className='flex items-center justify-end gap-3 text-sm text-zinc-500'>
                        Descuento Global (-)
                        <input type="text" className='py-1 px-2 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-[100px] lg:w-[150px] text-zinc-900 rounded-lg read-only:bg-zinc-200' readOnly />
                    </label>
                    <label className='flex items-center justify-end gap-3 text-sm text-zinc-500'>
                        Descuento por Item (-)
                        <input type="text" className='py-1 px-2 focus:outline-none focus:ring-0 focus:border-zinc-400 border border-zinc-300 w-[100px] lg:w-[150px] text-zinc-900 rounded-lg read-only:bg-zinc-200' readOnly />
            ...
            <button type='submit' className="text-white mt-5 bg-primary border hover:bg-primary/90 focus:outline-none font-medium rounded-xl text-sm px-5 py-3 text-center w-full">Guardar</button>
        </form>
    )
}
